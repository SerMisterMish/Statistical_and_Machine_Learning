...
\begin{frame}
    ...
    \begin{itemize}
        ...
        \[L _{MSE} = \frac 1 m \sum _{i = 1} ^m \left(\hat{y} ^{(i)} - y ^{(i)}\right) ^2\]
        
        \item \textbf{Для бинарной классификации}: Бинарная кросс-энтропия
        \[L _{BCE} = -\frac 1 m \sum _{i = 1} ^m \left[y ^{(i)} \log\left(\hat{y} ^{(i)}\right) + \left(1 - y ^{(i)}\right) \log\left(1 - \hat{y} ^{(i)}\right)\right]\]
    \end{itemize}
\end{frame}
